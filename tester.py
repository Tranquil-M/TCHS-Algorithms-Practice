...
def generate_grid() -> list:
    ...
    while DIFFICULTY >= 1:
        new_grid = [
            ["x" if i == 5 and j == 5 else "." for i in range(6)] for j in range(6)
        ]
        for _ in range(OBSTACLE_COUNT_BY_DIFFICULTY[DIFFICULTY]):
            new_obstacle = [random.randint(0, 5), random.randint(0, 5)]
            if new_obstacle[0] * 10 + new_obstacle[1] not in (
                0,
                55,
            ):  # invalid places (start and end)
...
def redraw():
    ...
    print_output += "\n".join(
        ["".join(map(str, print_grid[i])) for i in range(len(grid))]
    )
    ...
    if auto:
        ...
        i = input(
            "Press enter to continue, type 'auto' to automatically play, or type 'skip' to skip to the end of execution.\n"
        )
...
def move_forwards():
    ...
    new_pos = (
        position_x + MOVEMENT_VECTORS[direction][0],
        position_y + MOVEMENT_VECTORS[direction][1],
    )
...
def grade():
    ...
    print(
        "Your robot reached the exit!"
        if did_win
        else "Your robot failed to reach the exit."
    )
    score = (3 if did_win else 0) + fruits + (4 * DIFFICULTY if did_win else 0)
    print("Score: " + str(score))
    print(
        "win: "
        + str(3 if did_win else 0)
        + "/3"
        + ", apples: "
        + str(fruits)
        + "/6"
        + ", difficulty: "
        + str(4 * DIFFICULTY if did_win else 0)
        + "/8"
    )

    if score > 4:
        print(
            "You've received at least four points. Email your solution to tchsaero@tcusd.net to submit, and/or try for more points by improving your algorithm."
        )
    sys.exit()
